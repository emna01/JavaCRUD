...
public class JfMainProduto extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==btInserir){
			setVisible(false);
			JfInsertProduto jfinsert = new JfInsertProduto();
		...
		if(e.getSource()==btSelect){
			Cruds a = new Cruds();
			try {
				JOptionPane.showMessageDialog(null,a.ListarProduto());
		...
		if(e.getSource()==btDeletar){
			setVisible(false);
			JfDeleteProduto jfdelete = new JfDeleteProduto();
		...
		if(e.getSource()==btAlterar){
			setVisible(false);
			JfUpdateProduto jfupdate = new JfUpdateProduto();
