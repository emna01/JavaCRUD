...
public class JfMainGrupo extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==btInserir){
			setVisible(false);
			JfInsertGrupo jfinsert = new JfInsertGrupo();
		...
		if(e.getSource()==btSelect){
			Cruds a = new Cruds();
			try {
				JOptionPane.showMessageDialog(null,a.ListarGrupo());
		...
		if(e.getSource()==btDeletar){
			setVisible(false);
			JfDeleteGrupo jfdelete = new JfDeleteGrupo();
		...
		if(e.getSource()==btAlterar){
			setVisible(false);
			JfUpdateGrupo jfupdate = new JfUpdateGrupo();
