...
public class JfMainCidade extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==btInserir){
			setVisible(false);
			JfInsertCidade jfinsert = new JfInsertCidade();
		...
		if(e.getSource()==btSelect){
			Cruds a = new Cruds();
			try {
				JOptionPane.showMessageDialog(null,a.ListarCidades());
		...
		if(e.getSource()==btDeletar){
			setVisible(false);
			JfDeleteCidade jfdelete = new JfDeleteCidade();
		...
		if(e.getSource()==btAlterar){
			setVisible(false);
			JfUpdateCidade jfupdate = new JfUpdateCidade();
